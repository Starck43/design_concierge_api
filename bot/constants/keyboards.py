...
REG_GROUP_KEYBOARD = [["Дизайнер и/или специалист"], ["Поставщик товаров"]]
...
ORDER_ACTIONS_KEYBOARD = ["▶️ Разместить заказ", "⏹ Снять заказ", "👍 Принять предложение", "👎 Отклонить предложение", "📢 Сдать работу", "✅️ Принять работу", "❎ На доработку", "🏁 Досрочно завершить"]
ORDER_RESPOND_KEYBOARD = ["✅️ Откликнуться", "❎ Отозвать отклик", "🔔 Приступить к работе", "Открыть", "Показать", "⏳ Принять/Отклонить"]
